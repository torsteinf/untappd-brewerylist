...
const API = 'https://api.untappd.com/v4/brewery/info/'
const UNTAPPDSECRET = '&client_secret=' + process.env.REACT_APP_UNTAPPDSECRET
const UNTAPPDID = '?client_id=' + process.env.REACT_APP_UNTAPPDID
...
export class BreweryList extends Component {
  ...
  componentDidMount() {
    brewIds.map(brewIds => fetch(API + brewIds + UNTAPPDID + UNTAPPDSECRET )
     .then(res=>res.json())
     .then(obj=>{
